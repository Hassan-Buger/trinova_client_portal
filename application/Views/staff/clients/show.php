    ...
    <div id="addEntityModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(20,40,35,.45);backdrop-filter:blur(6px);z-index:99;align-items:center;justify-content:center;padding:20px">
        <div style="background:#fff;border-radius:24px;width:100%;max-width:680px;max-height:92vh;overflow:auto;padding:32px;box-shadow:0 24px 60px -28px rgba(0,0,0,.4);animation:tnpop .25s ease">
            ...
            <form action="/staff/clients/add-entity" method="POST" data-ajax-form>
                ...
                <select name="entity_scope" style="width:100%;padding:13px;margin-bottom:12px;border:1px solid #e0e9e5;border-radius:12px"><option value="company">Shared company record</option><option value="personal">Private personal record</option></select>
                        ...
                        <?php foreach ($entities as $b): ?>
                            <div style="padding:13px 0;border-bottom:1px solid rgba(20,60,50,.07)">
                                ...
                                <?php if(($b['entity_scope'] ?? '') === 'company'): ?>
                                    <div style="margin-top:12px;font-size:12px"><strong>Director / contact records</strong>
                                    <?php if(empty($contactsByEntity[(int)$b['id']]??[])): ?><div style="color:#8a9a94;margin-top:5px">No director records linked.</div><?php endif; ?>
                                    <?php foreach(($contactsByEntity[(int)$b['id']] ?? []) as $contact): ?><div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-top:6px;padding:7px 9px;background:#f7faf9;border-radius:9px"><span><strong><?= htmlspecialchars($contact['name']) ?></strong><?php if(!empty($contact['email'])): ?><br><span style="color:#61756e"><?= htmlspecialchars($contact['email']) ?></span><?php endif; ?></span><?php if(!empty($contact['needs_contact_details'])): ?><span style="white-space:nowrap;background:#fff2e4;color:#c5650b;padding:4px 7px;border-radius:999px;font-weight:700">Details required</span><?php else: ?><span style="white-space:nowrap;background:#e2f4ea;color:#27845b;padding:4px 7px;border-radius:999px;font-weight:700">Complete</span><?php endif; ?></div><?php endforeach; ?>
                                    </div>
                                    <div style="margin-top:12px;font-size:12px"><strong>Portal users with company access</strong>
                                    <?php foreach(($directorsByEntity[(int)$b['id']] ?? []) as $director): ?><form action="/staff/clients/unlink-director" method="POST" style="display:flex;justify-content:space-between;align-items:center;margin-top:5px"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(\Application\Core\Session::csrfToken()) ?>"><input type="hidden" name="entity_id" value="<?= (int)$b['id'] ?>"><input type="hidden" name="user_id" value="<?= (int)$director['id'] ?>"><input type="hidden" name="return_client_id" value="<?= (int)$client['id'] ?>"><span><?= htmlspecialchars($director['name']) ?> (<?= htmlspecialchars($director['email']) ?>)</span><button style="border:0;background:#fff1f0;color:#b42318;border-radius:8px;padding:4px 7px">Remove</button></form><?php endforeach; ?>
                                    <form action="/staff/clients/link-director" method="POST" style="display:flex;gap:6px;margin-top:8px"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(\Application\Core\Session::csrfToken()) ?>"><input type="hidden" name="entity_id" value="<?= (int)$b['id'] ?>"><input type="hidden" name="return_client_id" value="<?= (int)$client['id'] ?>"><select name="user_id" required style="min-width:0;flex:1"><option value="">Add director…</option><?php foreach($eligibleDirectors as $candidate): ?><option value="<?= (int)$candidate['id'] ?>"><?= htmlspecialchars($candidate['name']) ?> — <?= htmlspecialchars($candidate['email']) ?></option><?php endforeach; ?></select><button style="border:0;background:#0d9488;color:#fff;border-radius:8px;padding:5px 9px">Add</button></form>
                                    </div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
