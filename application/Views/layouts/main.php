...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* Premium staff & client workspace */
        :root {
            --portal-page: #f3f7f5;
            --portal-card: #ffffff;
            --portal-ink: #172d29;
            --portal-muted: #657771;
            --portal-subtle: #87958f;
            --portal-line: #dfe8e4;
            --portal-teal: #0d9488;
            --portal-teal-dark: #0b776f;
            --portal-teal-soft: #e9f6f2;
            --portal-warning: #c66b2b;
            --portal-warning-soft: #fff4e9;
            --portal-radius: 20px;
            --portal-shadow: 0 1px 2px rgba(18,55,48,.025), 0 18px 40px -34px rgba(18,55,48,.32);
            --sidebar-width: 258px;
            --sidebar-tablet-width: 80px;
            --drawer-width: 290px;
            --topbar-height: 88px;
            --topbar-mobile-height: 68px;
        }
        .tn-layout {
            display: flex;
            min-height: 100vh;
        ...
        .staff-sidebar, .client-sidebar, .tn-side {
            ...
            background: #fff;
            border-right: 1px solid rgba(42,77,68,.09);
            position: sticky;
            top: 0;
            z-index: 10;
        }
        .staff-sidebar__brand, .tn-sidebar-brand {
            display: flex;
            align-items: center;
            height: 72px;
        ...
        .staff-sidebar .tn-brand-logo, .client-sidebar .tn-brand-logo, .tn-side .tn-brand-logo {
            width: 184px;
            height: auto;
            max-height: 48px;
            object-fit: contain;
        ...
        .staff-sidebar nav, .client-sidebar nav, .tn-side nav {
            display: flex;
            flex: 1;
            flex-direction: column;
            gap: 4px;
        ...
        .staff-sidebar .tn-navitem, .client-sidebar .tn-navitem, .tn-side .tn-navitem {
            min-height: 46px;
            display: flex;
            align-items: center;
            gap: 12px;
        ...
        .staff-sidebar .tn-navitem svg, .client-sidebar .tn-navitem svg, .tn-side .tn-navitem svg {
            width: 20px;
            height: 20px;
            flex: 0 0 20px;
        }
        .staff-sidebar a.tn-navitem:hover, .client-sidebar a.tn-navitem:hover, .tn-side a.tn-navitem:hover {
            color: #21483f;
            background: #f2f8f6;
        }
        .staff-sidebar .tn-navitem.is-active {
            color: var(--portal-teal-dark);
            background: var(--portal-teal-soft);
            border-color: rgba(13,148,136,.09);
            font-weight: 750;
        }
        .staff-sidebar .tn-navitem:focus-visible, .client-sidebar .tn-navitem:focus-visible, .tn-side .tn-navitem:focus-visible {
            outline: 0;
        ...
        .staff-sidebar__logout, .client-sidebar form[action="/logout"] {
            margin: 14px 0 0;
            padding-top: 14px;
            border-top: 1px solid #edf2f0;
        }
        .staff-sidebar__logout button, .client-sidebar form[action="/logout"] button {
            width: 100%;
            background: transparent;
            cursor: pointer;
            text-align: left;
        }
        .staff-sidebar__logout button:hover, .client-sidebar form[action="/logout"] button:hover {
            color: #314a44;
            background: #f5f8f7;
        }
        .tn-topbar {
            ...
            border-bottom: 1px solid rgba(44,80,71,.045);
        ...
        /* TRINOVA-MOBILE-NAV-V2 */
        .tn-mobile-toggle, .mobile-menu-toggle {
            display: none;
            width: 44px;
            height: 44px;
            align-items: center;
            ...
            cursor: pointer;
            flex-shrink: 0;
            box-shadow: 0 4px 12px -8px rgba(18,55,48,.3);
            transition: background-color .18s ease, border-color .18s ease;
        }
        .tn-mobile-toggle:hover, .mobile-menu-toggle:hover {
            background: #f8fbfa;
            border-color: #cadbd5;
        }
        .tn-mobile-toggle span, .mobile-menu-toggle span {
            display: block;
            width: 20px;
            height: 2px;
            background: #354e47;
        ...
        .tn-mobile-toggle.is-active span:nth-child(1),
        body.tn-mobile-menu-open .tn-mobile-toggle span:nth-child(1),
        body.sidebar-open .mobile-menu-toggle span:nth-child(1) {
            transform: translateY(7px) rotate(45deg);
        }
        .tn-mobile-toggle.is-active span:nth-child(2),
        body.tn-mobile-menu-open .tn-mobile-toggle span:nth-child(2),
        body.sidebar-open .mobile-menu-toggle span:nth-child(2) {
            opacity: 0;
        }
        .tn-mobile-toggle.is-active span:nth-child(3),
        body.tn-mobile-menu-open .tn-mobile-toggle span:nth-child(3),
        ...
        .tn-mobile-overlay, .sidebar-overlay { position: fixed; inset: 0; background: rgba(18,40,35,.55); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); z-index: 40; opacity: 0; pointer-events: none; visibility: hidden; transition: opacity .25s ease, visibility .25s ease; }
        body.tn-mobile-menu-open .tn-mobile-overlay,
        body.sidebar-open .sidebar-overlay,
        .sidebar-overlay.is-visible { opacity: 1; pointer-events: auto; visibility: visible; }
        ...
        @media(max-width: 1180px) {
            ...
            .staff-metric { min-height: 145px; }
            .staff-dashboard__grid { grid-template-columns: minmax(0,1.45fr) minmax(290px,.85fr); }
        }

        @media(max-width: 900px) {
            .tn-profile { min-width: 0; padding-right: 6px; }
            .tn-profile__identity { display: none; }
            .staff-dashboard__grid { grid-template-columns: 1fr; }
            .staff-command { display: grid; grid-template-columns: minmax(250px,.9fr) minmax(300px,1.1fr); padding: 22px; gap: 0 20px; }
            .staff-command .staff-panel__header { grid-column: 1/-1; padding: 0 0 18px; }
            .staff-search, .staff-command__actions { padding: 0; }
            .staff-command__actions { padding-left: 0; }
            .staff-attention { grid-column: 1/-1; margin: 20px 0 0; }
        }

        /* Scoped intermediate tablet sidebar (only 769px to 1080px) */
        @media (min-width: 769px) and (max-width: 1080px) {
            .tn-navlabel, .tn-brandword, .tn-logout-label { display: none !important; }
            .tn-side { width: var(--sidebar-tablet-width) !important; align-items: center; padding: 20px 10px !important; }
        ...
        /* Mobile application layout & off-canvas drawer (<= 768px) */
        @media (max-width: 768px) {
            body { overflow-x: hidden; }
            .tn-layout { display: block !important; width: 100% !important; min-height: 100vh; min-height: 100dvh; }
            ...
            .tn-side .tn-navitem svg {
                width: 20px !important;
                height: 20px !important;
                flex: 0 0 20px !important;
            }
            .tn-navlabel, .tn-logout-label {
            ...
            .staff-sidebar__logout, form[action="/logout"] {
                display: block !important;
            ...
            .tn-topbar {
                ...
                background: rgba(243,247,245,.95) !important;
            ...
            #portal-content { padding: 14px 16px max(32px, env(safe-area-inset-bottom)) !important; }
            .tn-toast-stack { right: 12px; left: 12px; bottom: max(20px, env(safe-area-inset-bottom)); z-index: 95; }
            .tn-toast { min-width: 0; max-width: none; width: 100%; }
            .tn-notification-panel {
                position: fixed !important;
                top: calc(var(--topbar-mobile-height) + 4px) !important;
            ...
            .staff-dashboard__intro { margin-bottom: 16px; flex-direction: column; gap: 6px; }
            .staff-dashboard__date { display: none; }
            .staff-metrics { grid-template-columns: repeat(2,minmax(0,1fr)) !important; gap: 12px !important; margin-bottom: 18px !important; }
            .staff-metric { min-height: 128px; padding: 15px; border-radius: 17px; }
            ...
            .staff-panel__header { padding: 18px; }
            .staff-activity__list { padding: 0 18px 8px; }
            .staff-activity__item { grid-template-columns: 10px minmax(0,1fr); gap: 10px; padding: 12px 0; }
            ...
            .staff-activity__item time { grid-column: 2; justify-content: flex-start; margin-top: -6px; }
            .staff-command { display: block; padding: 18px; }
            .staff-command .staff-panel__header { padding: 0 0 16px; }
            .staff-command__actions { padding-top: 14px; }
            .staff-attention { margin-top: 16px; }
            .tn-client-toolbar { align-items: stretch !important; flex-direction: column !important; gap: 12px !important; }
            ...
            .tn-client-toolbar-actions form { width: 100% !important; display: flex !important; flex-direction: row !important; gap: 8px !important; }
            ...
            .tn-client-toolbar-actions a, .tn-create-client-button {
                width: 100% !important;
                display: flex !important;
            ...
            #clientsTable .client-row {
                ...
                border: 1px solid rgba(44,80,71,.08) !important;
                box-shadow: 0 4px 16px -10px rgba(16,54,45,.2) !important;
                position: relative !important;
                width: 100% !important;
                box-sizing: border-box !important;
            }
            #clientsTable .client-row td {
                display: flex !important;
                flex-direction: column !important;
            ...
            #clientsTable .client-row td::before {
            ...
            #clientsTable .client-row td:first-child, #clientsTable .client-row td:nth-child(2) {
            ...
            #clientsTable .client-row td:first-child::before, #clientsTable .client-row td:nth-child(2)::before {
                display: none !important;
            }
            #clientsTable .client-row .client-name, #clientsTable .client-row .client-email {
                overflow-wrap: anywhere !important;
                word-break: break-word !important;
                max-width: 100% !important;
            }
            #clientsTable .client-row td:last-child {
                margin-top: 10px !important;
                padding-top: 10px !important;
                border-top: 1px solid #f0f4f2 !important;
                width: 100% !important;
            }
            #clientsTable .client-row td:last-child > div {
                display: flex !important;
                width: 100% !important;
                gap: 10px !important;
            }
            #clientsTable .client-row td:last-child a {
                flex: 1 !important;
                justify-content: center !important;
            ...
            #clientsTable .client-row td:last-child button {
                width: 42px !important;
                height: 42px !important;
                flex: 0 0 42px !important;
                border-radius: 12px !important;
            }
            #clientsTable .tn-client-empty {
                display: block;
                background: #fff;
                border-radius: 20px;
            }
            #clientsTable .tn-client-empty td {
                display: block !important;
                width: 100%;
            }
        }
        ...
        @media (max-width: 480px) {
            ...
            .tn-welcome-role { display: none; }
            .tn-welcome-suffix { display: none; }
        }

        @media(prefers-reduced-motion:reduce) {
            .staff-sidebar .tn-navitem, .client-sidebar .tn-navitem, .tn-notification-button,
            .staff-panel__link svg, .staff-search button, .staff-action, .staff-action__arrow,
            .tn-side, .tn-mobile-overlay { transition: none !important; }
        }
    </style>
</head>
